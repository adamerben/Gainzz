    ...
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            <?php foreach ($exercises as $exercise): ?>
                <div
                    class="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-slate-100 flex flex-col group">
                    ...
                    <div class="relative h-48 bg-slate-900 overflow-hidden flex items-center justify-center">
                        ...
                        <div
                            class="absolute top-4 left-4 bg-orange-500 text-white text-xs font-black uppercase px-3 py-1.5 rounded-full shadow-md tracking-wider">
                            <?= htmlspecialchars($exercise['muscle_group_name'] ?? 'Neznámá partie') ?>
                        </div>
                    </div>

                    <div class="p-6 flex-grow flex flex-col">
                        <h3 class="text-xl font-black text-slate-800 mb-3 uppercase tracking-wide">
                            <?= htmlspecialchars($exercise['title']) ?></h3>

                        <div class="flex items-center gap-2 mb-4 text-xs text-slate-600 font-bold uppercase tracking-wider">
                            <span class="bg-slate-100 border border-slate-200 px-2 py-1 rounded">⚙️
                                <?= htmlspecialchars($exercise['equipment']) ?></span>
                            <span class="bg-slate-100 border border-slate-200 px-2 py-1 rounded">🔥
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-slate-100 mt-auto">
                            <a href="<?= BASE_URL ?>/index.php?url=exercise/show/<?= $exercise['id'] ?>"
                                class="text-orange-500 font-black hover:text-orange-600 uppercase text-sm tracking-widest flex items-center gap-1 transition-colors">
                            ...
                            <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
                                <div class="flex gap-4">
                                    <a href="<?= BASE_URL ?>/index.php?url=exercise/edit/<?= $exercise['id'] ?>"
                                        class="text-slate-400 hover:text-slate-800 transition-colors" title="Upravit">✏️</a>
                                    <a href="<?= BASE_URL ?>/index.php?url=exercise/delete/<?= $exercise['id'] ?>"
                                        onclick="return confirm('Opravdu chceš smazat tento cvik?')"
                                        class="text-slate-400 hover:text-rose-500 transition-colors" title="Smazat">🗑️</a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
